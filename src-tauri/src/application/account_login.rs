...
pub async fn login(
    ...
    let install_dir = ports.riot.install_path().await.and_then(|path| path.parent().map(|p| p.to_path_buf()));
    ...
    if sessions.has_snapshot(account_id) {
        sessions.restore(account_id, install_dir.as_deref())?;
    ...
    tokio::task::block_in_place(|| riot_login.login(pid, &username, &password))?;
    ...
    sessions.save(account_id, install_dir.as_deref())?;
...
mod tests {
    ...
    fn one_account_store() -> FakeAccountStore {
        FakeAccountStore {
            accounts: Mutex::new(vec![Account { id: "1".into(), label: "Main".into(), username: "forgotmyseed".into() }]),
